...
public class Crypto {
	...
	public Crypto(List<String> allKeys) {
		cryptoKeys = allKeys.stream().map(Key::fromBase64String).collect(Collectors.toList());
	...
	private static Key buildKey(String password, String salt) {
		...
		boolean pwHashWorked = scrypt.cryptoPwHashScryptSalsa208Sha256(pwHash, pwHash.length, pass, pass.length,
				encoder.decode(salt), Scrypt.SCRYPTSALSA208SHA256_OPSLIMIT_INTERACTIVE,
	...
	public String encrypt(String toEncrypt) {
		...
		boolean encryptionWorked = secretBox.cryptoSecretBoxEasy(cipherBytes, date, date.length, nonceBytes,
				currentKey.getAsBytes());
		if (!encryptionWorked) {
			return "ERROR"; //$NON-NLS-1$
		}
		byte[] noncePlusCipherBytes = Arrays.copyOf(nonceBytes, nonceBytes.length + cipherBytes.length);
		System.arraycopy(cipherBytes, 0, noncePlusCipherBytes, nonceBytes.length, cipherBytes.length);
	...
	public String decrypt(String encrypted) {
		...
		System.arraycopy(noncePlusCipherBytes, 0, nonceBytes, 0, SecretBox.NONCEBYTES);
		byte[] cipherBytes = new byte[noncePlusCipherBytes.length - SecretBox.NONCEBYTES];
		System.arraycopy(noncePlusCipherBytes, SecretBox.NONCEBYTES, cipherBytes, 0,
				noncePlusCipherBytes.length - SecretBox.NONCEBYTES);
		byte[] decrypted = new byte[cipherBytes.length - SecretBox.MACBYTES];
		boolean decryptionWorked = false;
		for (Key key : cryptoKeys) {
			decryptionWorked = secretBox.cryptoSecretBoxOpenEasy(decrypted, cipherBytes, cipherBytes.length, nonceBytes,
					key.getAsBytes());
			if (decryptionWorked) {
				break;
			}
	...
	public static String generateSalt() {
		return encoder.encode(lazySodium.randomBytesBuf(Math.toIntExact(Scrypt.SCRYPTSALSA208SHA256_SALT_BYTES)));
	}
